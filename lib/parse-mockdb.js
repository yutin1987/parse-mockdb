...
var UPDATE_OPERATORS = {
  ...
  Remove: function Remove(key, value) {
    ...
    var array = this[key];
    value.objects.forEach(function (object) {
      _this2[key] = _.reject(array, function (item) {
        return item === object;
      });
    });
  },
...
function handlePostRequest(request) {
  ...
  return promise.then(function () {
    ...
    var response = Object.assign(_.cloneDeep(_.omit(result, 'updatedAt')), { createdAt: result.createdAt.toJSON() });

    if (hooks.afterSave[className]) {
      var savedObject = Parse.Object.fromJSON(Object.assign({}, result, { className: className }));
...
function handlePutRequest(request) {
  var collection = getCollection(request.className);
  var currentObject = collection[request.objectId];
  var now = new Date();
  var data = request.data;

  var ops = extractOps(data);

  var updatedObject = Object.assign(_.cloneDeep(currentObject), data, { updatedAt: now });

  applyOps(updatedObject, ops);

  var object = new Parse.Object(request.className);
  ...
  return promise.then(function () {
    ...
    var response = Object.assign(_.cloneDeep(_.omit(object.toJSON(), ['createdAt', 'objectId'])), { updatedAt: now });
...
function fetchObjectByPointer(pointer) {
  ...
  return Object.assign({ __type: "Object", className: pointer.className }, _.cloneDeep(storedItem));
}
...
var QUERY_OPERATORS = {
  ...
  '$regex': function $regex(value) {
    var regex = _.clone(value).replace(QUOTE_REGEXP, "");
  ...
  '$select': function $select(value) {
    ...
    var objectMatches = _.filter(matches, function (match) {
